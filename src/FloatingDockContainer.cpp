...
#ifdef Q_OS_LINUX
	setWindowFlags(Qt::Window | Qt::WindowMaximizeButtonHint | Qt::WindowCloseButtonHint | Qt::WindowTitleHint | Qt::CustomizeWindowHint);
	activateWindow();
#else
...
#ifdef Q_OS_LINUX
void CFloatingDockContainer::hide(bool no_event){
	d->BlockHideEvent = no_event;
	Super::hide();
}

void CFloatingDockContainer::show(){
	// Prevent this window from showing in the taskbar and pager (alt+tab)
	internal::xcb_add_prop(winId(), "_NET_WM_STATE", "_NET_WM_STATE_SKIP_TASKBAR");
	internal::xcb_add_prop(winId(), "_NET_WM_STATE", "_NET_WM_STATE_SKIP_PAGER");
	Super::show();
}
#endif
...
void CFloatingDockContainer::hideEvent(QHideEvent *event)
{
	...
	// Prevent toogleView() events during restore state or at minimize
	if (d->BlockHideEvent || d->DockManager->isRestoringState())
	{
		d->BlockHideEvent = false;
		return;
	}
